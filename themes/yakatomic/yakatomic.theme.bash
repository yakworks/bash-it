...
# Modified from the default Atomic Bash Prompt for Bash-it
...
function ____atomic_top_right_parse() {
	...
	__TOP_RIGHT_LEN=$((__TOP_RIGHT_LEN + ${#args[1]} + ${#args[3]} + ${#args[4]} - 4))
	((__SEG_AT_RIGHT += 1))
}
...
# this is the start of the prompt and is just dashes
function ___atomic_prompt_user_info() {
	local color="${white?}" box
	# local info="${IYellow}\u${IRed} 🪂 ${purple}\h"
	# box="${normal?}${LineA?}\$([[ \$? != 0 ]] && echo \"${BIWhite?}[${IRed?}${SX?}${BIWhite?}]${normal?}${Line?}\")${Line?}${BIWhite?}[|${BIWhite?}]${normal?}${Line?}"
	box="${normal?}${LineA?}\$([[ \$? != 0 ]] && echo \"${BIWhite?}[${IRed?}${SX?}${BIWhite?}]${normal?}${Line?}\")${Line?}${BIWhite?}${normal?}"
...
function ___atomic_prompt_clock() {
	...
	local box="[|]" info
	info="$(TZ=UTC date +"${THEME_CLOCK_FORMAT}")"
	printf "%s|%s|%s|%s" "${color}" "⌚️${info} UTC" "${bold_white?}" "${box}"
}

function ___atomic_prompt_uinfo() {
	local color="${white?}"
	local box="[|]" info
	# info="${IYellow}\u${IRed} 🪂 ${purple}\h"
	#  for now blank out unifo as it never really changes
	info="${IYellow}\h"
	printf "%s|%s|%s|%s" "${color}" "${info}" "${bold_white?}" "${box}"
...
function ___atomic_prompt_k8s_context() {
	...
	local color="${white?}"
	local box="[|]" info
	info="${purple}🎯${kubernetes_context}"
	printf "%s|%s|%s|%s" "${color}" "${info}" "${bold_white?}" "${box}"
}
...
___ATOMIC_TOP_RIGHT="exitcode python k8s_context uinfo todo clock battery"
...
function __atomic_ps1() {
	printf "%s%s%s" "\n$(____atomic_top)" "$(____atomic_bottom)" "${normal?}"
}
